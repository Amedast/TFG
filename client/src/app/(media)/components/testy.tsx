...
export default function AddToList ({
  ...
  const openModal = async () => {
    if (isLogged) {
      ...
      if (isInList) {
        const contentInList = res.data.content
        setStatus(contentInList?.status.toString())
        setRating(contentInList?.rating)
        setProgress(contentInList?.progress)
        setStartDate(contentInList?.startDate)
        setEndDate(contentInList?.endDate)
        setNotes(contentInList?.notes)
      }
      setIsModalOpen(true)
    } else {
